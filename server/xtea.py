...
def xtea_encrypt(key, block, n=64, endian="!"):
    ...
    """
        ...
        * key = 128 bit (16 char)
        * block = 64 bit (8 char)
        * n = rounds (default 32) #changed to 64
    ...
    # print ("v0 and v1 are " + str(v0) + " " + str(v1))
    # print ("Key  is " + key)
    # key = b'key'
    k = struct.unpack(endian + "4L", key)
    sum, delta, mask = 0, 0x9e3779b9, 0xffffffff # removed trailing L from all values
...
def xtea_decrypt(key, block, n=32, endian="!"):
    ...
    v0, v1 = struct.unpack(endian + "2L", block)
    k = struct.unpack(endian + "4L", key)
    delta, mask = 0x9e3779b9, 0xffffffff #removed trailing L from both values
    sum = (delta * n) & mask
    for round in range(n):
        v1 = (v1 - (((v0 << 4 ^ v0 >> 5) + v0) ^ (sum + k[sum >> 11 & 3]))) & mask
        sum = (sum - delta) & mask
...
if __name__ == '__main__':
    ...
    data == xtea_cbc_decrypt(key, iv2, xtea_cbc_encrypt(key, iv1, data))
