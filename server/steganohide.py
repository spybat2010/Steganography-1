...
def as_bits(data):
    ...
    return (ord(char) >> shift & 1
            for char, shift in itertools.product(data, list(range(7, -1, -1))))
...
def hide_msg(image, data):
    ...
    def hide_bits(pixel, bits):
        ...
        return tuple(itertools.starmap(set_least_sig_bit, list(zip(pixel, bits))))

    hdr = as_32_bit_string(len(data))
    # print "length of the data as 32 bit string is " + str(hdr)
    payload = '%s%s' % (hdr, data)
    n_pxls = image.size[0]*image.size[1]
    n_bnds = len(image.getbands())
    # print "Number of pixel components ", n_bnds
    # print("len of pxl and bns " + str(n_pxls*n_bnds))
    # print("len of payload " + str(len(payload)))
    if len(payload)*6 <= n_pxls*n_bnds:
        ...
        # (tupled by izip)
        # print "payload bits are " , str(payload_bits)
        # print zip(img_data, payload_bits)
        new_img_data = itertools.starmap(
            hide_bits, zip(img_data, payload_bits))
        # create the image with the manipulated least significant bits
        image.putdata(list(new_img_data))
        return image
    else :
        print("wtf happend hereeeeeeeeeeeeee")
